...
import {requestBulkCards,API_BASE,} from "../../../api/competitions";
...
function CardView({ data }) {
  ...
  const styleNorm = String(style || kind || discipline || "").toLowerCase();

  const isPoomsae =
    styleNorm === "poomsae" ||
    styleNorm.includes("poom") ||
    styleNorm.includes("پومسه") ||
    Boolean(poomsae_style || poomsae_type || poomsae_type_display || age_category_name);
  ...
  const categoryValue = isPoomsae
    ? poomsae_style || poomsaeTypeFa || "—"
    : weight_name || "—";
    ...
    <div className="enroll-card enroll-card--outlined" style={{ marginBottom: 24 }}>
      ...
      <div className="enroll-card__grid enroll-card__grid--photo">
        ...
        <div className="enroll-card__info-cols">
          ...
          {isPoomsae ? (
            <>
              <Info label="سبک مسابقه" value={categoryValue} />
              <Info label="گروه سنی" value={ageValue} />
            </>
          ) : (
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const response = await requestBulkCards(
          idsToLoad,
          { kind }
        );
        ...
        const responseMap = new Map(
          list.map((item) => [
            Number(item?.enrollment_id),
            item,
          ])
        );

        const out = idsToLoad.map((rawId) => {
          ...
          if (!item) {
            return {
              id,
              status: "error",
              message: "اطلاعات این کارت از سرور دریافت نشد.",
            };
          }

          if (item.error === "not_ready") {
            return {
              id,
              status: "pending",
              message:
          ...
          if (item.error === "forbidden") {
            return {
              id,
              status: "error",
              message: "اجازه مشاهده این کارت را ندارید.",
            };
          }
          ...
          if (item.error === "not_found") {
            return {
              id,
              status: "error",
              message: "کارت این ثبت‌نام یافت نشد.",
            };
          }
